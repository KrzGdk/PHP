  ...
  if(isset($_POST['sent'])){
    ...
    if(array_key_exists($user_name, $assoc)){
      ...
      if(md5($pass) === $info[1]){
        ...
        $i = 0;
        do{
          $blog_file_name = $date.$time.$sec.str_pad($i, 2, 0, STR_PAD_LEFT);
          $i++;
        }
        ...
        foreach ($_FILES as $form_name => $file_data) {
          if($file_data['error'] === 0){
            $save_path = $blog_path.DS.$blog_file_name.'.'.pathinfo($file_data['name'], PATHINFO_EXTENSION);
...
<div id="main">
  ...
  <form method="POST" enctype="multipart/form-data">
    ...
    <div class="field">
      ...
      <input type="text" name="date" value="<?php echo $date ?>" disabled><br>
    </div> 
    <div class="field">
      <div class="label">Godzina:</div>
      <input type="text" name="time" value="<?php echo $time ?>" disabled><br>
    </div> 
    <div class="field">
